...
public class redplayer1 : MonoBehaviour {
	...
	void Start () {
		other = GameObject.Find("bluecube(Clone)");
		othertext = GameObject.Find ("bluetext").guiText;
		spawner = GameObject.Find("redspawner");
		onGround = false;
		doubleJump = false;
		tripleJump = false;
	}
	...
	void FixedUpdate () {
		...
		Ray ray1 = new Ray(transform.position + new Vector3(transform.localScale.x/2, 0, 0), -transform.up);
		RaycastHit rayHit1 = new RaycastHit();
		
		Ray ray2 = new Ray(transform.position - new Vector3(0, transform.localScale.x/2, 0), -transform.up);
	...
	void OnCollisionEnter(Collision collision){
		...
		else if (collision.gameObject == GameObject.Find("platform")){
			onGround = true;
		}
	}
